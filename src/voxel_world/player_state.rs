...
impl PlayerState {
    ...
    pub fn put_staff(&mut self, id: usize) -> Option<(usize, Option<usize>, usize)> {
        for i in 0..10 {
            return if let (Some(old_id), num) = self.toolbar[i] {
                if old_id != id || num == MAX_STAFF_FIXED {
                    continue;
                }
                self.toolbar[i] = (Some(id), num + 1);
                Some((i, Some(id), num + 1))
            } else {
...
impl StoragePlayerState for MapDataBase {
    fn save_player_state(
        &mut self,
        username: String,
        player_state: PlayerState,
